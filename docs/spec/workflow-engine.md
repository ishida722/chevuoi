...
ルーターは候補から名前を 1 つ選ぶだけなので、既定では軽量モデル（`haiku`）で動きます。`config.toml` の `[router]` で変更できます:

```toml
[router]
model = "haiku"   # 既定値。エイリアスの解決先を固定したければ完全名（例: "claude-haiku-4-5-20251001"）
# model = ""      # 空文字列にすると --model を付けず Claude Code の既定モデルで動く
```

`[router] model` は `claude --model` に渡す値で、`[llm] model`（`ctx.llm` 用の langchain モデル ID）とは別物です。ルーターは `ctx.llm` を使わないため、`[llm]` の有無や値は選択に影響しません。

ルーターが使うモデルは起動時レポート（§9）と `vuoi workflow select` の出力に表示されます。設定漏れで意図しないモデルが使われていないかは、まずここで確認してください（タイトルに `[intent]` マーカーがあるカードは LLM を呼ばずに決まるため、表示されたモデルは使われません）。
...
この出力が**コードを 1 行も実行せずに得られる**のが、メタデータを TOML に置く最大の利点です。先頭行にはルーターが `claude --model` に渡すモデルを出します（`[router] model = ""` のときは `未指定（Claude Code の既定モデル）`）。設定漏れのまま重いモデルで回り続けても気づけない、という事故を防ぐためです。
