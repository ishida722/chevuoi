...
- 代表カードへの集約コメントは冪等キーを持つ。`🤖 triage:` 行に `key=<digest 12 桁>/<畳まれる側のカード ID>` を含め、投稿前に代表カードの直近コメントを照合する（代表カードのみ 1 回の追加 API 呼び出し）。digest はタイトルと本文だけの関数なので、完全一致の重複カード同士では等しくなり、digest だけでは別カードの集約コメントを抑止してしまう。照合はコメント本文の部分一致なので、鍵の前後（`key=` と改行）まで含めて照合する（カード ID は可変長で、ある ID が別の ID の接頭辞になり得る）。
- 畳まれる側・アーカイブされる側のカード自身に残す理由コメントは照合しない（追加の API 呼び出しを代表宛の 1 回に抑えるため）。アーカイブに失敗して再開したランでは同じ理由コメントが重ねて付く。
